...
> row trx_id也是有极限的，2^48-1，如果大于这个值的话，可能会出现脏读的情况，但是这种几率几乎等于不可能，只读操作不会分配trx_id，分配的trx_id会自动增加一个2^48，为了和只读的做个区分。
...
在RR隔离级别下，readview会在当前事务首次执行SELECT快照读的时候创建。即事务1先执行，但是没有继续SELECT快照读操作，此时事务2插入了一条记录并提交，事务1是可以看到事务2插入的数据的。因为按照MVCC实现原理，当事务1查询事务2插入的行时，该行记录上的隐藏事务id字段并不会在事务1新创建的readview活跃事件列表中，而且小于readview创建的高水位，所以是可见的。在RR隔离级别下，当前事务没有提交，快照读操作始终读到的都是第一次执行select找到的快照版本。倘若其他事务执行了Insert操作，而当前事务执行了任何的当前读操作（update，delete，select .. from update等）,还是会出现幻读。
...
```mysql
mysql>select id from t where c in(5,20,10) lock in share mode;

mysql>select id from t where c in(5,20,10) order by c desc for update;
```

这两个语句本质上其实一个加了排序一个没加排序。但是如果这两个语句一起执行的话，就容易导致死锁。

先来说一下第一个语句的执行步骤

1. 在查找c=5的时候，先锁住了(0,5]，但是c（非唯一索引），为了确认还有没有其他记录c=5，就向右遍历直到下一个值c=10才能确认没有了，这时候加了间隙锁(5,10)。
2. 同样c=10,c=20的时候也是这样的范围。

这个过程是**一个一个**加的，那么如果这两个语句同时执行的话， 按照第二条记录从20开始上锁，很有可能导致死锁。<u>即语句2想去给5上锁的时候，5已经被语句1上锁了，语句1想去给10上锁的时候，10已经被语句2锁住了。</u>因此考虑执行语句的时候，尽量保证一个字段同一种排序方式。
...
按照分析来说，10的记录在session A的语句中，不应该被锁住，A语句的锁住的两个间隙应该是（5，10）（10，15），但是由于delete操作删除了id=10这一行，那么原来的两个间隙就由（10，15）扩张为（5，15）。

结论：所谓间隙，其实根本是由这个间隙右边的那个记录来定义的。
